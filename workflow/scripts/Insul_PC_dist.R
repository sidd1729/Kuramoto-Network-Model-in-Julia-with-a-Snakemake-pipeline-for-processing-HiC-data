...
svg(filename = str_glue("/home/ksslab/Siddharth/Program_Directory/Kuramoto_Processing_Pipeline/temporaries/trials/R_trials/{resolution}/PC1_{resolution}_adjust_1_75.svg"))#,res = 300)
#svg(str_glue("/home/ksslab/Siddharth/Program_Directory/Kuramoto_Processing_Pipeline/temporaries/trials/R_trials/{resolution}/Rao_{resolution}.svg"))
...
for (file in list.files(str_glue("/home/ksslab/Siddharth/Program_Directory/Kuramoto_Processing_Pipeline/results/PCs/{resolution}"))){
    ...
    if ((substr(file, str_length(file)-10, str_length(file)-8) ==  "PC1") & (str_split(file,'_')[[1]][length(str_split(file,'_')[[1]])-1] == str_glue('{resolution}'))){
    #if (file[-11:-8:+1] == "PC1") & 
        #print(file)
        count <- count + 1
        ...
        density_plot <- density(trial_nanless,adjust = 1.75)
        if (count == 1){
        #par(mar = c(2,2,2,2))
        plot(density_plot$x,density_plot$y,type = "l",col = colors_vec[count],xlab ="PC1",ylab = "Density of PC1",ylim = c(0,1.2),cex.axis = 0.5)}
        else{
...
title(str_glue("PC1 distribution for various time points at {resolution} bin size with 1.75 adjust"))
