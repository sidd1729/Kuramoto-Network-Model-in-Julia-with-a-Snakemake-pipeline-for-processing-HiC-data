...
write.csv(DLR_df,str_glue("~/Siddharth/Program_Directory/Kuramoto_Processing_Pipeline/results/CSV_files/{df_type}.csv"))
DLR_choose <- names(DLR_df)[c(4,5,6,7,8)]
} else if (df_type == "RNA_seq"){
DLR_df <- read_feather("~/Siddharth/Program_Directory/Kuramoto_Processing_Pipeline/results/RNA_seq_processing/Kramer_2021_subset_tpm_means.feather")
DLR_choose <- names(DLR_df)[c(3,4,5,6,7)]
}
...
if (df_type == "RNA_seq"){
  len_unique_row <- apply(DLR_df_omit,1,function (row) length(unique(row))) #Lambda function equivalent first taking unique elements of row and length of the row
  DLR_df_omit <- DLR_df_omit[len_unique_row > 3,]
  DLR_df_omit <- DLR_df_omit %>% 
   filter(!near(ctrl_mean, 0) | !near(hpi_24_mean, 0) | !near(hpi_36_mean, 0) | !near(hpi_72_mean, 0),!near(dpi_5_mean, 0))
   DLR_df_omit_filtered <- DLR_df_omit[DLR_choose]
   row.names(DLR_df_omit_filtered) <-DLR_df_omit$gene_name
   svg("~/Siddharth/Program_Directory/Kuramoto_Processing_Pipeline/results/RNA_seq_processing/test_heatmap.svg")
   p1<-pheatmap(DLR_df_omit_filtered,cluster_row = TRUE, cluster_cols = FALSE,show_rownames = FALSE,treeheight_row = 0, treeheight_col = 0,scale = "row")
   dev.off()}
...
cluster_centre_calc <- function(center_no,clustered_df = DLR_df_bound){#,column_names = c("control_100kb","hpi_12_100kb","dpi_2_100kb","dpi_4_100kb","dpi_7_100kb")){
  ...
  #1st line below for cooltools, 2nd on for dchic,3rd for K_50 and DLR, 4th for RNA_seq
  ...
  #mean_obj <- cluster_specific_df %>% summarise(across(c(ctrl_mean,hpi_24_mean,hpi_36_mean,hpi_72_mean,dpi_5_mean), ~mean(.x,na.rm = TRUE)))
  print(mean_obj)
...
#Replace when using RNA seq and other timepoint data
names(df_test) <- c("control","12hpi","2dpi","4dpi","7dpi")#c("control","24hpi","36hpi","72hpi","5dpi")#
...
mfuzz.plot2(standardized,cl=cl3,mfrow=c(4,2),time.labels=c("control","24hpi","36hpi","72hpi","5dpi"),x11 = FALSE,centre = TRUE,centre.col = "blue")#c("control","12hpi","2dpi","4dpi","7dpi"),x11 = FALSE,centre = TRUE,centre.col = "blue")
